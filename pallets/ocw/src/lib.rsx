...
pub mod pallet {
	...
	//use serde_json::{Deserialize, Deserializer};
	...
	const HTTP_REMOTE_REQUEST_FOR_DOT: &str = "https://api.coincap.io/v2/assets/polkadot";
	const HTTP_HEADER_USER_AGENT_FOR_DOT: &str = "cc";
	...
	pub mod crypto {
		use crate::KEY_TYPE;
		use frame_support::{
			sp_runtime::app_crypto::{app_crypto, sr25519},
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn fetch_price_info() -> Result<(), Error<T>> {
			...
			match Self::fetch_n_parse_for_dot() {
				Ok(dot_price_info) => {
					println!("{:?}",dot_price_info);
				}
				Err(err) => {
					return Err(err);
				}
			...
			log::info!("cached gh-info: {:?}", 1);

			let price = Self::fetch_n_parse_for_dot()?;
			log::info!("cached price-price: {:?}", price);
		...
		fn fetch_n_parse_for_dot() -> Result<DotPriceInfo, Error<T>> {
			let resp_bytes = Self::fetch_from_remote_for_dot().map_err(|e| {
				log::error!("fetch_from_remote error: {:?}", e);
				<Error<T>>::HttpFetchingError
			})?;
		...
		/// This function uses the `offchain::http` API to query the remote github information,
		///   and returns the JSON response as vector of bytes.
		fn fetch_from_remote_for_dot() -> Result<Vec<u8>, Error<T>> {
			log::info!("sending request to: {}", HTTP_REMOTE_REQUEST_FOR_DOT);

			// Initiate an external HTTP GET request. This is using high-level wrappers from `sp_runtime`.
			let request = rt_offchain::http::Request::get(HTTP_REMOTE_REQUEST_FOR_DOT);
			...
			//   See: https://developer.github.com/v3/#user-agent-required
			let pending = request
				.add_header("User-Agent", HTTP_HEADER_USER_AGENT_FOR_DOT)
				.deadline(timeout) // Setting the timeout time
				.send() // Sending the request out by the host
				.map_err(|_| <Error<T>>::HttpFetchingError)?;
