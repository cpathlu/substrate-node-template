...
const HTTP_HEADER_USER_AGENT: &str = "cpath";

const FETCH_TIMEOUT_PERIOD: u64 = 3000; // in milli-seconds
const LOCK_TIMEOUT_EXPIRATION: u64 = FETCH_TIMEOUT_PERIOD + 1000; // in milli-seconds
const LOCK_BLOCK_EXPIRATION: u32 = 3; // in block number

const HTTP_REMOTE_FOR_DOT_PRICE: &str = "https://api.coincap.io/v2/assets/polkadot";
...
impl<T: Config> Pallet<T> {
	...
	fn fetch_price_info() -> Result<(), Error<T>> {
		let price = Self::get_remote_price().map_err(|_| Error::<T>::HttpFetchingError)?;
	...
	fn get_remote_price() -> Result<(u64, Permill), Error<T>> {

		let request = rt_offchain::http::Request::get(HTTP_REMOTE_FOR_DOT_PRICE);
		...
		let price_str =
			str::from_utf8(&price_info.data.priceUsd).map_err(|_| <Error<T>>::HttpFetchingError)?;

		let price = Self::parse_price_from_str(price_str).map_err(|_| <Error<T>>::HttpFetchingError)?;
	...
	// fn parse_price(s: &str) -> Result<(u64, Permill), Error<T>> {
